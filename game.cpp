...
Game::Game(QWidget *parent ):QGraphicsView(parent)
{
    ...
    gameScene->setSceneRect(0,0,1400,1100);
...
void Game::drawChessBoard()
{
    ...
    drawDeadHolder(0,0,Qt::darkGray);
    drawDeadHolder(1100,0,Qt::darkGray);
...
{
    deadHolder = new QGraphicsRectItem(x,y,300,1000);
    QBrush brush;
    brush.setStyle(Qt::SolidPattern);
...
void Game::displayMainMenu()
{
    ...
    //Create the title
    QGraphicsTextItem *titleText = new QGraphicsTextItem("OOP Project");
    ...
    //create Button
    /*Button * creditbutton = new Button("Play again?");
    int cxPos = width()/2 - creditbutton->boundingRect().width()/2;
    int cyPos = 450;
    creditbutton->setPos(cxPos,cyPos);
    connect(creditbutton,SIGNAL(clicked()) , this , SLOT(start()));
    addToScene(creditbutton);
    listG.append(creditbutton);*///use later if sir asks
